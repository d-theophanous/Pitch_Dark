...
namespace Geecku.DefaultNetworking.Template
{
    ...
    public class GameMessageTemplate : Singleton<GameMessageTemplate>, IGameMessage //- ToDO: eine Singleton GameMessage classe erstellen!
    {
        #region Links
        ...
        public void SetTransforms(Transform server_transform, Transform client_transform)
        {
            ...
            var server = ServerTransform.gameObject.AddComponent<DataObjectTemplate>();
            (Server = server).Init(true);
            var client = ClientTransform.gameObject.AddComponent<DataObjectTemplate>();
            (Client = client).Init(false);
        }
        #endregion

        public static DataObjectTemplate Client { get; private set; }
        public static DataObjectTemplate Server { get; private set; }
